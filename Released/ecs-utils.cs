    ...
    while (bayQueue.Any() || activeBays.Any())
    {
        ...
        foreach (var bay in activeBays.ToList())
        {
            ...
            if (activationTimes[bay] > activationDelay)
            {
                if (bay.Projector != null && IsValidBlock(bay.Projector))
                {
                    if (!bay.Projector.IsProjecting || bay.Projector.RemainingBlocks == 0 || bayProgress[bay] > hardLimitCycles)
                    {
                        foreach (var hardpoint in bay.Hardpoints)
                        {
                            if (!creativeMode)
                ...
                else
                {
                    foreach (var hardpoint in bay.Hardpoints)
                    {
                        if (IsValidBlock(hardpoint.Projector))
                        {
                            if (!hardpoint.Projector.IsProjecting || hardpoint.Projector.RemainingBlocks == 0 || bayProgress[bay] > hardLimitCycles)
                            {
                                if (!creativeMode)
                                {
                                    hardpoint.Connector.Connect();
...
IEnumerator<bool> ReloadDDs(List<string> types)
{
    ...
    while (tempHydrogenTanks.Count > 0 && runCounter < 180)
    {
        ...
        {
            tempHydrogenTanks[i].Stockpile = true;
            if (!IsValidBlock(tempHydrogenTanks[i]) || tempHydrogenTanks[i].FilledRatio > tankFillRatio)
            {
                tempHydrogenTanks[i].Stockpile = false;
